...
$f_{\mathbb{M}}$|`f_mb`|Fraction of extinct and extant taxa formed by the BD process
...
```
f(n, N) := rhs(solve(N = 2 * ((1 + q) ^ n), q)[1]); plot3d(f(n, N), [n, 1, 4], [N, 2, 4], [zlabel, "q"]);
...
calc_q <- function(n_nu_events, n_mb_taxa) {
  ...
  (2.0 ^ (1.0 / n_nu_events))
  # if (q < 0.0) {
  #   print(paste(n_nu_events, n_mb_taxa))
  #   q <- 0.0
  # }
  testit::assert(q >= 0.0)
  testit::assert(q <= 1.0)
