...
$f_{\dagger}$|`f_extinct`|Fraction of taxa that become extinct
...
```{r}
df <- expand.grid(q = c(0.0, 0.5, 1.0), n_nu_events = c(1, 2, 3, 4, 16))
df$n_mb_taxa <- NA
...
```
f(n, N) := rhs(solve(N = 2 * ((1 + q) ^ n), q)[1]); plot3d(f(n, N), [n, 9, 16], [N, 15, 500], [zlabel, "q"]);
```
...
## 4. fraction of the taxa created by the BD process

The number of taxa created by the BD process is simply:
...
## 6. number of extinct taxa

The number of taxa gone extinct is simply:
...
$nu$ must have such a value that the desired number of MB events $n_{\nu}$
is attained.
...
```{r}
knitr::kable(
  dplyr::select(df, c("n_extant_taxa", "f_extinct", "f_mb", "n_nu_events", "lambda", "mu", "nu", "q"))
)
```
