...
In `folder_name`:
  ...
  * `1`
     ...
     * `mbd.trees`: the posterior trees from `mbd.tree`, created by `raz_create_posterior_files("mbd.tree")`
     * `mbd.log`: the posterior parameter estimates from `mbd.tree`, created by `raz_create_posterior_files("mbd.tree")`
     * `mbd_mar_lik.csv`: the posterior's marginal likelihood from `mbd.tree`, created by `raz_create_posterior_files("mbd.tree")`
     * `bd.trees`: the posterior trees from `bd.tree`, created by `raz_create_posterior_files("bd.tree")`
     * `bd.log`: the posterior parameter estimates from `bd.tree`, created by `raz_create_posterior_files("bd.tree")`
     * `bd_mar_lik.csv`: the posterior's marginal likelihood from `bd.tree`, created by `raz_create_posterior_files("bd.tree")`
