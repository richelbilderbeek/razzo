...
test_ML1 <- mbd::mbd_ML(idparsopt = idparsopt, 
                        ...
                        initparsopt = (initparsopt <- (start_pars <- c(0.3, 0.1, 3, 0.15))[idparsopt]),
                        brts = (brts <- mbd::mbd_sim(pars = (sim_pars <- c(0.2, 0.15, 2, 0.15)), 
                                                     soc = (soc <- 2), 
                                                     age = (age <- 10), 
                                                     cond = (cond <- 1),
                                                     tips_interval = (tips_interval <- c(0,70))
                        )$brts), 
...
### 3 par test
rm(list = ls()); idparsopt <- 2:4; set.seed(2)#set.seed(length(idparsopt))
...
### 4 par test
rm(list = ls()); idparsopt <- 1:4; set.seed(4)#; set.seed(length(idparsopt))
